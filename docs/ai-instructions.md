...
- **Key Features**:
  ...
  - **Database Cleanup**: Removes orphaned hash-course mappings periodically to keep the database clean.
...
- **Performance and Scalability**:
  ...
  - Cleanup is handled by `purgemappings_task.php`, which runs periodically to remove orphaned mappings, reducing overhead.
...
2. **Tasks**:
   ...
   - **`purgemappings_task.php`**: Scheduled task that periodically removes orphaned hash-course mappings from `autotranslate_hid_cids` for non-existent courses, course modules, and course sections.
...
- **Database Cleanup**:
  - The `purgemappings_task.php` scheduled task removes orphaned hash-course mappings from `autotranslate_hid_cids` for non-existent courses, course modules, and course sections, running daily at midnight to ensure database consistency without the timing issues of event-driven observers.
...
6. **Performance Optimization**:
   - Monitor the performance of `tagcontent_task.php`, `fetchtranslation_task.php`, and `purgemappings_task.php` with large datasets, adjusting `managelimit`, `fetchlimit`, and `batchsize` as needed.
